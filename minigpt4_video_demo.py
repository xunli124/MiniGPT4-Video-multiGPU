...
def generate_prediction(video_path, instruction,
                        gen_subtitles=True, stream=False,
                        minigpt4_gpu_id=1, whisper_gpu_id=0, answer_module_gpu_id=1,
                        max_frames=16):
    ...
    # Limit frames
    total_frames = len(prepared_images)
    if total_frames > max_frames:
        indices = torch.linspace(0, total_frames - 1, max_frames).long()
        prepared_images = prepared_images[indices]
        total_frames = max_frames

    print(f"Number of frames: {total_frames}")
    print(f"Image tensor shape: {prepared_images.shape}")
    print(f"Image tensor dtype: {prepared_images.dtype}")
    print(f"Image tensor range: [{prepared_images.min()}, {prepared_images.max()}]")

    target_gpu = answer_module_gpu_id  
    
    model.to(f"cuda:{target_gpu}")
    
    prepared_images = prepared_images.unsqueeze(0).to(f"cuda:{target_gpu}")
    
    conv = CONV_VISION.copy()
    conv.system = ""
    
    placeholder_count = prepared_instruction.count('<Img><ImageHere>')
    if placeholder_count != total_frames:
        prepared_instruction = prepared_instruction.replace('<Img><ImageHere>', '')
        placeholders = ' '.join(['<Img><ImageHere>'] * total_frames)
        prepared_instruction = prepared_instruction + ' ' + placeholders
    ...
    try:
        ...
        answers = model.generate(
            prepared_images, prompt, max_new_tokens=args.max_new_tokens, 
            do_sample=True, lengths=[total_frames], num_beams=1
        )
        return answers[0]
    except Exception as e:
        ...
        try:
            ...
            answers = model.generate(
                prepared_images, prompt, max_new_tokens=min(args.max_new_tokens, 100), 
                do_sample=False, lengths=[total_frames], num_beams=1
            )
            return answers[0]
        except Exception as e2:
            return f"Error: {e2}. Please check the model configuration and input data."
...
title = """<h1 align="center">MiniGPT4-video </h1>"""
description = """<h5>This is the demo of MiniGPT4-video Model.</h5>"""
project_details="""<div style="text-align: center;">
        <div>
            <font size=3>
                <div>
                    <a href="https://vision-cair.github.io/MiniGPT4-video/">Project Page</a>
...
video_path=""
with gr.Blocks(title="MiniGPT4-video",css=text_css ) as demo :
    gr.Markdown(title)
